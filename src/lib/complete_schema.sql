-- ==========================================
-- STB FF GUILD - COMPLETE DATABASE SCHEMA
-- ==========================================
-- Run this entire script in your Supabase SQL Editor to set up the database.

-- 1. Enable UUID Extension (Required for unique IDs)
...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    squad_id UUID, -- Foreign key added later to avoid circular dependency
...
-- Add Foreign Key to profiles for squad_id if not exists
DO $$
BEGIN
...
-- 2.3. GUILD CONFIGURATION (Global Stats)
...
-- 2.4. SITE SETTINGS (Dynamic Website Config)
...
CREATE TABLE IF NOT EXISTS public.notices (
    ...
    title TEXT NOT NULL,
    content TEXT,
    category TEXT DEFAULT 'General', -- 'General', 'Important', 'Urgent'
    type TEXT DEFAULT 'general', -- For UI styling
...
CREATE TABLE IF NOT EXISTS public.messages (
    ...
    channel TEXT DEFAULT 'global', -- 'global', 'squad', 'rules'
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Insert default Guild Config if not exists
...
-- Allow admins to insert profiles (for manual adds)
...
-- Read: Global/Rules for everyone, Squad for squad members
...
-- Insert: Auth users for global, Squad members for squad, Admins for rules
...
-- 5.8. OTHER POLICIES (Tournaments, Finances, Logs)
